...
// @ts-ignore
import { accounts } from './protocol/test-wallets.js';
import { eEthereumNetwork, eNetwork, eOtherNetwork, ePolygonNetwork } from './protocol/helpers/types';
import { BUIDLEREVM_CHAINID, COVERAGE_CHAINID } from './protocol/helpers/buidler-constants';
...
if (!SKIP_LOAD) {
  ['misc', 'migrations', 'dev', 'full', 'helpers'].forEach(
    (folder) => {
      const tasksPath = path.join(__dirname, 'protocol', 'tasks', folder);
...
require(`${path.join(__dirname, 'protocol', 'tasks/misc')}/set-bre.ts`);
